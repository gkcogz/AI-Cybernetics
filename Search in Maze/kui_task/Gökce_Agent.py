...
class Agent:
    ...
    def find_path(self) -> list[State]:
        ...
        # state itself, next_state (=None since the start state has no parent, g_score) 
        
        open_list =[(self.heuristic(start, goal), next(itertools.count()), start, None, 0)]
        ...
        # The values are unpacked into '-' (ignored h-score), 'current_state (=start)', 'parent state', 
        # and g_score. 
        while open_list:
            _, _, current_state, next_state, g_score = heapq.heappop(open_list)
            ...
            # Maps the 'current_state' to its 'next_state' to reconstruct the path later.
            came_from[current_state] = next_state
            ...
            if self.environment.is_goal(current_state):
                ...
                # 'path' is a list of 'State' objects.
            ...
            for action in self.environment.get_actions(current_state):
                ...
                # The algorithm ensures that 'open_öist' remains a min-heap based on the 'f' score.
                # Thus A* algorithm efficinently selects the next node the explore.
                heapq.heappush(open_list, (new_f, next(itertools.count()), new_state, current_state, new_g))
            
            # Here, we visualize the found path. Path, here is the list of state objects.
            # texts={state: f for _, state, _, f in open_list}, we created a dictionary to display f_score on the path.
                
            #self.environment.render(current_state=current_state, texts={state: f for _, state, _, _, f in open_list}, colors={state: f for _, state, _, _, f in open_list}, wait=True)
            self.environment.render(current_state=current_state, next_states=[new_state], wait=True)
